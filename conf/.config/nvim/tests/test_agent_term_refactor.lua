...
draft.open_input_buffer({ target_bufnr = term, draft_height = 15 })
vim.cmd("resize 7")
assert_eq(vim.api.nvim_win_get_height(0), 7, "draft resize setup")
draft.hide()
assert_eq(vim.t.agent_input_height, 7, "draft height saved")
local ok, msg = draft.focus_or_open({ target_bufnr = term, draft_height = 15 })
assert(ok, msg)
