...
function M.send_command(target, command, opts)
  ...
  opts = opts or {}
  local add_newline = opts.add_newline ~= false
  local exclude_current = opts.exclude_current or false
  local paste = opts.paste == true
  ...
  local success_msg = string.format("Command sent to terminal %s (%s)", tostring(target), terminal.name)
...
function M.list_terminals()
  ...
  local result = {}
  for i, term in ipairs(M.get_all_terminals()) do
    table.insert(result, {
      index = i,
      bufnr = term.bufnr,
      name = term.name,
