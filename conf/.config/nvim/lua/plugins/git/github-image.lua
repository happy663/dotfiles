return {
	{
		...
		opts = function(_, opts)
			...
			local function github_image_upload()
				...
				elseif
					bufname:match("octo://")
					and (bufname:match("/issue/") or bufname:match("/issues/") or bufname:match("/pull_request/"))
				then
					local parts = vim.split(bufname, "/")
					if #parts >= 6 then
						local owner = parts[3]
				...
				vim.fn.jobstart(cmd, {
					...
					on_exit = function(_, code)
						if code ~= 0 then
							local tail = vim.list_slice(stderr_lines, math.max(1, #stderr_lines - 4))
							local msg = "画像アップロードに失敗しました (exit " .. code .. ")"
							if #tail > 0 then
								msg = msg .. "\n" .. table.concat(tail, "\n")
							end
